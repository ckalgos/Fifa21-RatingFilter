...
// @version      1.0.4
// @updateURL    https://github.com/ckalgos/Fifa-AutoBuyer/blob/master/autobuyer.js
...
(function () {
  ...
  JSUtils.inherits(
    UTSearchEnhancerViewController,
  ...
  db.transaction(function (tx) {
    tx.executeSql(
      "CREATE TABLE IF NOT EXISTS PlayerId (id, nationid,leagueid,teamid)"
    );
  ...
  db.transaction(function (tx) {
    tx.executeSql(
      "SELECT * FROM CurrentRating",
      [],
      function (tx, ratingResults) {
  ...
  UTItemService.prototype.searchTransferMarket = function searchTransferMarket(
    searchCriteria,
    pageNum,
    fromRequestItem,
    observable
    ...
    function successSearch(param1, successResult) {
      ...
      if (successResult.success && successResult.response.items.length > 0) {
        this.marketRepository.setPageCache(
          pageNum,
          successResult.response.items,
          Date.now(),
          successResult.maxAge > 0 ? successResult.maxAge : null
        );
      }
    ...
    function refreshSuccess(currRefresh, refreshResult) {
      ...
      response.success = !![];
      response.status = refreshResult.success
        ? HttpStatusCode["OK"]
    ...
    if (!this.marketRepository.isCacheExpired(pageNum)) {
      ...
      var currConfigObj = getAppMain()
        .getConfigRepository()
        .getConfigObject(EAConfigurationRepository.KEY_ITEMS_PER_PAGE);
      var currItemCount = JSUtils.isValid(currConfigObj)
        ? currConfigObj[
            EAConfigurationRepository.ITEMS_PER_PAGE.TRANSFER_MARKET
          ]
        : 20;
      searchCriteria.offset = currItemCount * (pageNum - 1);
      searchCriteria.count = currItemCount + 1;
      accessobjects.Item.searchTransferMarket(searchCriteria).observe(
        this,
        successSearch
      );
    }
  ...
  UTItemService.prototype.updateCriteria = function updateCriteria(
    searchCriteria,
    l,
    result
  ) {
    db.transaction(
      function (tx) {
        tx.executeSql(
          `SELECT id FROM PlayerId WHERE (${searchCriteria.nation} IS -1 OR nationid = ${searchCriteria.nation}) AND
          ...
          function (tx, results) {
            ...
            if (playerIds.length) {
              searchCriteria.maskedDefId =
                playerIds[getRandNumber(0, playerIds.length - 1)];
            }
  ...
  UTMarketSearchResultsViewController.prototype._searchFut = function (l) {
    this._paginationViewModel.stopAuctionUpdates(),
      services.Item.searchTransferMarket(this._searchCriteria, l, true).observe(
        this,
        function _onRequestItemsComplete(e, t) {
          ...
          if (0 < this._searchCriteria.offset && 0 === t.data.items.length)
            ...
            var s = this.getView(),
              a = null;
            if (
              (!this._stadiumViewmodel ||
                (this._searchCriteria.type !== SearchType.VANITY &&
                  this._searchCriteria.type !== SearchType.CLUB_INFO &&
                  this._searchCriteria.type !== SearchType.BALL) ||
                (a = this._stadiumViewmodel.getStadiumProgression(
                  this._searchCriteria.subtypes
                )),
              ...
              JSUtils.isValid(r)
                ? this._pinnedListItem.setItem(r)
                : this._paginationViewModel.setPinnedItem(this._compareItem);
            } else
              !isPhone() &&
                0 < o.length &&
                s.selectListRow(this._paginationViewModel.getCurrentItem().id);
          }
    ...
    if (window.hasLoadedAll && jQuery(".ut-app-settings-actions").length) {
      if (jQuery(".ut-app-settings-actions").first().length) {
        if (!jQuery("#snipe_rating_filter").length) {
          jQuery(".ut-app-settings-actions")
            .first()
            .append(
              `<div class="ut-item-search-view">
                           <div class="search-price-header">
                            ...
                            <div style="width:100%" class="buttonInfo"> 
                                    <div class="inputBox">
                                        <input placeholder="-1" id="ab_player_rating" type="tel" class="numericInput" value=` +
                window.currentRating +
                `></div>
                                    </div> 
                            </div>
  ...
  window.saveSettings = function () {
    ...
    db.transaction(function (tx) {
      ...
      if (rating) {
        tx.executeSql(
          `INSERT INTO CurrentRating (rating) VALUES (${rating})`,
          [],
          function (tx, ratingResults) {},
          null
        );
  ...
  window.fetchPlayers = function (rating) {
    jQuery.getJSON(
      `content/${fut_guid}/${fut_year}/fut/items/web/players.json`,
      function (res) {
        let playerFilteredIds = [];

        var legendReduced = res.LegendsPlayers.reduce(function (
          filtered,
          option
        ) {
        ...
        for (let i = 0; i < playerFilteredIds.length; i++) {
          services.Item.requestItemByDefId(playerFilteredIds[i]).observe(
            ...
            function (sender, response) {
              ...
              db.transaction(function (tx) {
                tx.executeSql(
                  `INSERT INTO PlayerId (id,nationid,leagueid,teamid) Values (${playerFilteredIds[i]},${item.nationId},${item.leagueId},${item.teamId})`,
                  [],
                  function (tx, ratingResults) {}
                );
              });
